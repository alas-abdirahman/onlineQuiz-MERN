...
import { deleteUser } from '../redux/thunk/users.thunk';
...
export default function UserPage() {
  ...
  const handleDeleteUser = async () => {
    const res = await dispatch(deleteUser({ _id: selectedID, accessToken }));
    if (res.type === "user/delete-user/fulfilled") {
      setOpen(null);
      toast.success("User Deleted");
      dispatch(getUsers({ page: 0, accessToken }));
      navigate("/dashboard/user");
    } else if (res.type === "user/delete-user/rejected") {
      toast.error(`${res.error.message}`);
    }
  };

  const handleEditUser =  async () => {
    setCurrentUser(users.admins.find((user) => user._id === selectedID));
    setOpen(null);
    setNewAdminModal(true);
  };
  const [openNewAdminModal, setNewAdminModal] = useState(false);
  const handleOpen = () => {setNewAdminModal(true); setCurrentUser(null)};
      ...
      <Container>
        ...
        >
          <Fade in={openNewAdminModal}>
            <Box sx={style}>
              <NewUserForm handleClose={handleClose} currentUser={currentUser}/>
            </Box>
          </Fade>
        </Modal>
          ...
          <Scrollbar>
            <TableContainer sx={{ minWidth: 800 }}>
              <Table>
                ...
                />
                <TableBody>
                  {users.admins.map((row) => {
                    const { _id, fullname, email, tel, username, password, status } = row;
                    ...
                    return (
                      <TableRow hover key={_id} tabIndex={-1} role="checkbox" selected={selectedUser}>
                        ...
                        <TableCell align="left">
                          <Label color={(status === 'blocked' && 'error') || 'success'}>{sentenceCase(status)}</Label>
                        </TableCell>
